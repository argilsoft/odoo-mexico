...
class ir_attachment_facturae_mx(osv.Model):
    ...
    def signal_printable(self, cr, uid, ids, context=None):
        ...
        try:
            ...
            report_ids = report_multicompany_obj.search(cr, uid, [('model', '=', 'account.invoice')], limit=1) or False
            ...
            #print "report_service: ", report_service
            result, format = self.pool['report'].get_pdf(cr, uid, [invoice.id], report_name, context=context), 'pdf'
            result = base64.b64encode(result)
            report_name = invoice.fname_invoice
            if not report_name:
